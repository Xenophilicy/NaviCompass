...
use pocketmine\event\player\{PlayerDeathEvent, PlayerItemUseEvent, PlayerJoinEvent, PlayerQuitEvent, PlayerRespawnEvent};
...
class NaviCompass extends PluginBase implements Listener{
	...
	public array $compassCooldown = [];
	private array $queryResults;
	private array $list;
	private EnchantmentInstance $enchInst;
		...
		foreach(self::$settings["List"] as $target){
			...
			switch($mode){
				case "ext":
					...
					$this->startQueryTask($value[2], (int)$value[3]);
					break;
				case "int":
				case "wd":
					if(isset($value[3])){
	...
	private function createEnchant() : void{
		EnchantmentIdMap::getInstance()->register(100, new Enchantment("NaviCompass", Rarity::MYTHIC, ItemFlags::ALL, ItemFlags::ALL, 5));
		$enchantment = EnchantmentIdMap::getInstance()->fromId(100);
		...
		foreach($this->list as $target){
			...
			switch(strtolower($value[0])){
				...
				case "int":
					...
					$this->getServer()->getWorldManager()->loadWorld($value[2]);
					$level = $this->getServer()->getWorldManager()->getWorldByName($value[2]);
					if(is_null($level)){
						$worldPlayerCount = 0;
					}else{
						$worldPlayerCount = sizeof($level->getPlayers());
					}
					$subtext = str_replace("{current-players}", (string)$worldPlayerCount, $subtext);
					if(isset($value[3])){
						$search = $value[3];
						$file = $value[4];
					}
			...
			if(isset($search)){
				if($search == "url"){
					$form->addButton($value[1] . "\n" . $subtext, 1, "https://" . $file);//why use http?
				}
				if($search == "path"){
...
//        $this->getServer()->getNetwork()->broadcastPacket([$player], $sound);
	}
